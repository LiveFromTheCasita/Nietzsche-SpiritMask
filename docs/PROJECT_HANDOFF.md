...
## What remains
The remaining defined work is now concentrated in the book-expansion roadmap (WP-08), final sitewide editorial integration (WP-19), and the release gate (WP-20). WP-17 analytics/privacy verification is complete. WP-15 verified search, sitemap, internal links, course navigation, fragments, and canonicals across the full public site; WP-16 found no required accessibility remediation; WP-18 established the canonical deployment and rollback workflow in `DEPLOYMENT_RUNBOOK.md`. WP-19 corrections are prepared for independent review on `integrator/wp-19-editorial-integration`; see `WP19_INTEGRATION_REVIEW.md`. WP-20 remains blocked. See `WORK_PLAN.md`.
