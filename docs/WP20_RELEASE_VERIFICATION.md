...
- **Immediate previous production deployment at account-check time:** WP-19 content release `dpl_38uK9ZY2XxVjLdsBHm2hbJtTCTFP` at `3a72aec`. Vercel marks it as a rollback candidate and shows an enabled **Instant Rollback** action. Returning from the PR #53 docs-only deployment to it would not undo the WP-19 public-content changes. Merging this WP-20 documentation PR will create another production deployment and change which deployment is immediately previous; check the live control again before any future action.
